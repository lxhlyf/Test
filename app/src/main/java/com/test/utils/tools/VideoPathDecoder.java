 ...
 *
 * 用于解析JieCaoVideoPlayer的播放地址的工具类
 */
...
public abstract class VideoPathDecoder {
    ...
    public void decodePath(String srcUrl) {
        ...
        settings.setCacheMode(WebSettings.LOAD_NO_CACHE); //设置为没有缓存
...
//        webView.loadUrl("file:///android_asset/parse.html");
    ...
    private void sendRequest(String srcUrl,String r,String s) {
        RetrofitEngine.getInstance().getApiService().getVideoPath(srcUrl,r,s) .subscribeOn(Schedulers.io())
                ...
                .subscribe(new Subscriber<VideoPathResponse>() {
                    ...
                    public void onError(Throwable e) {
                        Log.e("videoDecoder——",e.getLocalizedMessage());
                        onDecodeError();
                    }

                    @Override
                    public void onNext(VideoPathResponse response) {
                        Log.e("parseVideoResponse: ", ""+response);
                        String url = "";
